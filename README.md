...
- **事業内容（SERVICE）**: 太陽光発電・蓄電池・エコキュート・セントラル浄水器・住宅リフォーム・空き家対策の6事業を、画像付きカードの設計図風グリッドで表示（WORKSと同じ画像を流用）。加えてガスの切り替え・携帯電話・ネット回線の見直しを軽量なタグ一覧として併記
...
太陽光発電・エコキュート・セントラル浄水器・蓄電池は `画像素材/` フォルダにいただいた実際の施工写真（`public/static/images/works/solar.jpg`, `ecocute.jpg`, `purifier.jpg`, `battery.jpg`）を使用しています。
残り2種（外壁塗装・空き家）は実写真が届き次第、`public/static/images/works/*.svg` を実写真に差し替え、`WORKS` の `image` パスを更新してください。
...
「トータルサポートで一貫したサービス」の柱のみ写真が届いていないため、`.philosophy-closing` として画像なしの結びの文章にしています。
写真が届いたら `public/static/images/philosophy/` に配置し、`src/index.tsx` の `philosophy-closing` を他の `philosophy-row` と同じマークアップ（`philosophy-row-img` を追加）に変更してください。

## 差し替えが必要なプレースホルダー素材
以下は仮のSVGプレースホルダーです。実素材が揃い次第、差し替えてください（ロゴ・太陽光・エコキュート・セントラル浄水器・蓄電池は差し替え済み）。
| ファイル | 用途 |
|---------|------|
