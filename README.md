...
- **事業内容（SERVICE）**: 太陽光発電・蓄電池・エコキュート・オール浄水器・住宅リフォーム・空き家対策の6事業を設計図風グリッドで表示。加えてガスの切り替え・携帯電話・ネット回線の見直しを軽量なタグ一覧として併記
...
残り4種（蓄電池・オール浄水器・外壁塗装・空き家）は実写真が届き次第、`public/static/images/works/*.svg` を実写真に差し替え、`WORKS` の `image` パスを更新してください。
...
| `public/static/images/works/purifier.svg` | オール浄水器の施工事例画像 |
...
- [x] 実ロゴ画像への差し替え
- [x] ヒーロー動画の実素材への差し替え
- [ ] 残り4種（蓄電池・オール浄水器・外壁塗装・空き家）の実施工写真への差し替え
